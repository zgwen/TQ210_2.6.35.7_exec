...
{
    //创建设备，自动分配主设备号
    major = register_chrdev(0,"keysignal_drv",&key_fops);
    //两部自动创建设备节点
    //1.创建设备类，可在/sys/class目录下查看该类文件夹
    key_class = class_create(THIS_MODULE,"keysignal_drv");
    //2.在/dev目录下创建节点
    key_class_dev = device_create(key_class,NULL,MKDEV(major,0),NULL,"key_signal"); 
